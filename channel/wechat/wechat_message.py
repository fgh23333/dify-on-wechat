...
class WechatMessage(ChatMessage):
    def __init__(self, itchat_msg, is_group=False):
        ...
        if itchat_msg["Type"] == TEXT:
            if itchat_msg["MsgType"] == 49:  # 引用消息
                self.is_reply = True
                self.ctype = ContextType.TEXT
                self.content = itchat_msg["Text"]
                try:
                    appmsg = itchat_msg.get("Content")
                    root = ET.fromstring(appmsg)
                    self.quote_content = root.find("appmsg").find("title").text
                except Exception as e:
                    logger.error(f"parse quote msg error: {e}")
                    self.quote_content = ""
            else:
                self.ctype = ContextType.TEXT
                self.content = itchat_msg["Text"]
